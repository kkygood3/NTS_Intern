...
function mapPriceData(data, prices) {
    ...
    // checkType
    data.productPrices.forEach(function (item) {
    ...
    data.productPrices.forEach(function (item) {
        ...
        var currentType = pricingType[item.priceTypeName];
        var currentString = "";
...
SlidingAnimation.prototype.init = function (optionals) {
    ...
    this.imageList.forEach(function (item) {
        if (item == _this.imageList[0]) {
            _this.imageList[0].style.left = 0 + "px";
        }
        else {
            item.style.left = _this.SLIDE_CONATINER_WIDTH + "px";
        }
    ...
    SlidingAnimation.prototype.startAutoAnimation = function () {
        ...
        setTimeout(function () {
            requestAnimationFrame(function () { return _this.slide({
                isAutoStart: true,
                isReverse: false,
                isResizing: false
            }); });
...
SlidingAnimation.prototype.slide = function (params) {
    ...
    var prevImage = this.imageList[this.prevSlideCount];
    var currentImage = this.imageList[this.currentSlideCount];
    var nextImage = this.imageList[this.nextSlideCount];
    ...
    for (var iter = 0; iter < this.animationSpeed; iter++) {
        if (params.isReverse) {
    ...
    if (needToStop) {
        if (this.isAutoStart) {
            setTimeout(function () {
                requestAnimationFrame(function () { return _this.slide(params); });
            }, this.animationStopDuration);
        }
        else {
            this.isAnimating = false;
...
function arrayToElementRenderer(data, target, item, opt, prototype) {
    ...
    Handlebars.registerHelper("scoreToDouble", function (item) {
        return item.toFixed(1);
    });
    Handlebars.registerHelper("isImagePresent", function (item) {
        return item.length > 0 ? "" : "no_img";
    });
    Handlebars.registerHelper("commentImageUrl", function (item) {
        return item[0].saveFileName;
    });
    Handlebars.registerHelper("rsvDateSplit", function (item) {
        return item.split(" ")[0];
    });
    Handlebars.registerHelper("rsvIdFix", function (item) {
        var width = 8;
        item = item + '';
        return item.length >= width ? n : new Array(width - item.length + 1).join('0') + item;
    });
    Handlebars.registerHelper("fileToBlob", function (item) {
        return window.URL.createObjectURL(item);
    });
    Handlebars.registerHelper("commentImageId", function (item) {
        return item.commentImages[0].imageId;
    });
    var parser = new DOMParser();
    var parsedItems = parser.parseFromString(bindTemplate({ data: list }), "text/html");
    var elementClassName = parsedItems.querySelector("body").firstElementChild.className;
    var newCommentItems = parsedItems.querySelectorAll("." + elementClassName);
    newCommentItems.forEach(function (item, i) {
        target.append(item);
        if (prototype) {
            prototype(item, i);
        }
...
function logout() {
    document.querySelector(".logout").addEventListener("click", function () {
        var request = new XhrRequest("POST", "./api/logout");
        request.setCallback(function () { alert("Logout"), window.location.href = "./"; });
        request.send();
    });
}
function scrollToTopAttacher() {
    document.querySelectorAll(".gototop").forEach(function (item) {
