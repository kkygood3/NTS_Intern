				...
				setTimeout(()=>{
					document.querySelectorAll("#"+reservationType.toLowerCase()+"_list .btn").forEach((v)=>{
						v.addEventListener("click", (event)=>{
							...
							reservationCancel.reservationInfoId = parseInt(target.name);
							reservationCancel.productId = parseInt(target.id);
							reservationCancel.description = target.parentElement
																  .parentElement
																  .firstElementChild
																  .nextElementSibling
																  .innerText;
							
							cancelButtonDown(reservationCancel, reservationType);
					    });
					});
				}, 100);
			}
...
function getReservationInfoMap(reservationInfo, reservationType){
	if(reservationType === "SCHEDULED"){
		...
		cancelBtn = cancelBtn.replace("_{name}", reservationInfo.reservation.reservationInfoId);
		cancelBtn = cancelBtn.replace("_{productId}", reservationInfo.displayInfo.productId);
