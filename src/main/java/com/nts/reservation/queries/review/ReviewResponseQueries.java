...
public class ReviewResponseQueries {
	public static String SELECT_REVIEW = 
		"SELECT display_info.id AS display_info_id,reservation_user_comment.comment,file_info.save_file_name AS comment_image, reservation_info.reservation_name, reservation_info.reservation_date,reservation_user_comment.score,product.description AS product_description, average_score_subquery.average_score" + 
		" FROM display_info " + 
		...
		" INNER JOIN (SELECT display_info.id, ROUND(AVG(reservation_user_comment.score),1) AS average_score FROM reservation_user_comment " + 
